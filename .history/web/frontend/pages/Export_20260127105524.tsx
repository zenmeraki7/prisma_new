...
/* ========================================================================== *
 * Types to mirror GraphQL shape (for type-safety)
 * ========================================================================== */

interface ExportJobNode {
  id: string;
  name: string;
  format: string;
  scope: string;
...
interface ExportJobsQueryResult {
  exportJobs: {
    edges: { cursor: string; node: ExportJobNode }[];
    pageInfo: {
      hasNextPage: boolean;
...
export default function ExportPage() {
  ...
  const { selectedProductIds } = useProductsSelectionStore();
  const { planHash, filterExpr } = useFilterContextStore();

  const selectedCount = selectedProductIds.length;

  // History query for FIRST page + cursor-based pagination.
  // We always query from the beginning (`after: null`) and use fetchMore
  // to append pages. This keeps polling cheap and avoids re-querying
  // arbitrary cursors on every "Load more".
  const {
    data,
    ...
    stopPolling,
    fetchMore,
  } = useQuery<ExportJobsQueryResult>(EXPORT_JOBS_QUERY, {
  ...
  const [startExportJob, { loading: startExportLoading }] = useMutation(
    START_EXPORT_JOB_MUTATION,
    {
      onCompleted: () => {
        ...
        // Light refetch to pull the new job into the first page.
        refetch();
      },
      onError: () => {
  ...
  // History edges (all pages currently in cache)
  const historyEdges = data?.exportJobs?.edges ?? [];

  // Derived: is there any active job (PENDING/RUNNING) in the visible set?
  ...
  // Status-aware polling: only poll the FIRST page while active jobs exist.
  // This avoids hammering the API and keeps things cheap:
  // - new jobs show up on the first page
  // - active jobs (recent) are on the first page
  React.useEffect(() => {
    if (hasActiveJobs) {
      startPolling(5000);
    } else {
  ...
  // Pagination info from the latest snapshot
  ...
  const handleLoadMore = React.useCallback(() => {
    ...
    fetchMore({
      ...
      // Append new edges to existing ones in cache:
      updateQuery: (prev, { fetchMoreResult }) => {
        if (!fetchMoreResult) return prev;

        return {
          exportJobs: {
            __typename: prev.exportJobs.__typename,
            edges: [
              ...prev.exportJobs.edges,
              ...fetchMoreResult.exportJobs.edges,
            ],
            pageInfo: fetchMoreResult.exportJobs.pageInfo,
          },
        };
      },
    });
  ...
  // Memoized history rows for DataTable
  const historyRows: React.ReactNode[][] = React.useMemo(
    () =>
      historyEdges.map(({ node }) => {
        const size = formatFileSize(node.fileSizeBytes ?? null);
  ...
  const handleExport = React.useCallback(async () => {
    ...
    if (scope === "selected") {
      ...
      input.selectedProductIds = selectedProductIds;
    }
