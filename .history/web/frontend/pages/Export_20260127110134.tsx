...
export default function ExportPage() {
  ...
  // 🧠 Selection store: branded IDs for UI, GraphQL-safe IDs for mutations
  const { selectedProductIds, asGraphqlProductIds } =
    useProductsSelectionStore();

  // 🧠 Filter context: branded PlanHash / FilterExprJson for UI,
  // GraphQL-safe representation for mutations
  const { planHash, filterExpr, asGraphqlFilterContext } =
    useFilterContextStore();
  ...
  const handleExport = React.useCallback(async () => {
    ...
    if (scope === "filtered") {
      const { planHash: gqlPlanHash, filterJson } =
        asGraphqlFilterContext();

      if (!gqlPlanHash || !filterJson) {
        setShowErrorBanner(true);
        return;
      }

      input.planHash = gqlPlanHash;
      input.filterJson = filterJson;
    }
