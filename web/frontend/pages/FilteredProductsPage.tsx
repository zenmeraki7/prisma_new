...
function buildFilterExprFromUi(params: {
  ...
  if (params.productType.trim()) {
    children.push(leaf("product.productType", "contains", params.productType.trim()));
  }
...
// React Query hook to get plan for applied filter
function usePlanFilter(app: any, filterExpr: FilterExpr | null) {
  return useQuery({
...
// Infinite scroll hook for products by filter
...
export default function FilteredProductsPage() {
  const app = useAppBridge(); // automatically inferred
          ...
          <Card>
            <Box padding="400">
              <InlineStack align="space-between" gap="300" blockAlign="center">
                ...
                <InlineStack gap="200">
                  <Button onClick={handleApplyFilters} loading={applying} primary>
                    Apply
                  </Button>
              ...
              {showSnapshotBanner && (
                <Box paddingBlockStart="200">
                  <Banner tone="info">
                    <p>
                      This filter requires SNAPSHOT mode (planHash <code>{planHash}</code>). Backend evaluates
                      via BulkOperations and materializes the result; later you can switch to{" "}
                      <code>productsBySnapshot</code> + <code>snapshotStatus</code> for large sets.
                    </p>
                  </Banner>
                </Box>
              )}
              ...
              >
                {allItems.map((product, index) => (
                  ...
                  >
                    ...
                    <IndexTable.Cell>
                      <Text as="span">{product.tags.length ? product.tags.join(", ") : "—"}</Text>
                    </IndexTable.Cell>
                    <IndexTable.Cell>
                      {product.hasImages ? <Badge tone="success">Yes</Badge> : <Badge tone="critical">No</Badge>}
                    </IndexTable.Cell>
                    <IndexTable.Cell>
